...
/*   Updated: 2024/03/14 23:17:21 by misidori         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	DiamondTrap	player("DiamondTrap");
	std::string	answer;
	bool		use_shield = true;
	...
	while (player.get_energy_points() > 0 && player.get_hit_points() > 0)
	{
		player.print_stats();
		std::cout << std::endl;
		std::cout << YELLOW << "[ATTACK] [BE REPAIRED] [GUARD GATE] [HIGH FIVE GUYS] [WHO AM I] [EXIT]" << RESET << std::endl;
		std::cout << "Choose your next move: ";
		std::getline(std::cin, answer);
		if (std::cin.eof())
			return ;
